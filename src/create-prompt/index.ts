...
Images from comments have been saved to disk. Paths are in the formatted content above. Use Read tool to view them.
  ...
  // Use GITHUB_RUN_NUMBER which is the UI run number (not the internal GITEA_RUN_ID)
  const jobUrl = `${GITEA_SERVER_URL}/${context.repository}/actions/runs/${process.env.GITHUB_RUN_NUMBER || process.env.GITEA_RUN_ID || ""}`;
