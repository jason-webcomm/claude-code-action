...
async function getOrCreateBranchRef(
  ...
  const refUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/refs/heads/${branch}`;
  ...
  if (refResponse.ok) {
    const refData = (await refResponse.json()) as GiteaRef;
  ...
  const baseRefUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/refs/heads/${baseBranch}`;
  ...
  if (!baseRefResponse.ok) {
    ...
    // Try default branch
    const defaultRefUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/refs/heads/${defaultBranch}`;
    ...
    const defaultRefData = (await defaultRefResponse.json()) as GiteaRef;
    baseSha = defaultRefData.object.sha;
  } else {
    const baseRefData = (await baseRefResponse.json()) as GiteaRef;
    baseSha = baseRefData.object.sha;
  }

  // Create the new branch using the same pattern as git
  const createRefUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/refs`;
...
server.tool(
  ...
  async ({ files, message }) => {
    ...
    try {
      ...
      const commitUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/commits/${baseSha}`;
      ...
      const commitData = (await commitResponse.json()) as GiteaCommit;
      ...
      const treeEntries = await Promise.all(
        processedFiles.map(async (filePath) => {
          ...
          if (isBinaryFile) {
            ...
            const blobUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/blobs`;
      ...
      // 4. Create a new tree
      const treeUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/trees`;
      ...
      const treeData = (await treeResponse.json()) as GiteaTree;

      // 5. Create a new commit
      const newCommitUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/commits`;
      ...
      const newCommitData = (await newCommitResponse.json()) as GiteaNewCommit;

      // 6. Update the reference to point to the new commit
      const updateRefUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/git/refs/heads/${branch}`;

      // We're seeing intermittent 403 errors on certain repos when updating git references
      // These appear to be transient API issues that succeed on retry.
