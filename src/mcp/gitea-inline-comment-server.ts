...
if (!REPO_OWNER || !REPO_NAME || !PR_NUMBER) {
  console.error(
    "[Gitea Inline Comment Server] Error: REPO_OWNER, REPO_NAME, and PR_NUMBER environment variables are required",
  );
  process.exit(1);
}

// Gitea Inline Comment MCP Server - Provides inline PR comment functionality
...
server.tool(
  ...
  "Create an inline comment on a specific line or lines in a PR file",
  {
    ...
    body: z
      .string()
      .describe(
        "The comment text (supports markdown and Gitea code suggestion blocks). " +
  ...
  async ({ path, body, line, startLine, side, commit_id }) => {
    try {
      ...
      // Sanitize the comment body to remove any potential tokens
      ...
      // Fetch PR to get the head commit SHA if not provided
      let commitSha = commit_id;
      if (!commitSha) {
        const prUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/pulls/${pull_number}`;
        ...
        if (!prResponse.ok) {
          throw new Error(
            `Failed to fetch PR: ${prResponse.status} - ${await prResponse.text()}`,
          );
        }
      ...
      // Gitea API endpoint for creating line comments
      // POST /repos/{owner}/{repo}/pulls/{index}/comments
      const commentUrl = `${GITEA_API_URL}/repos/${owner}/${repo}/pulls/${pull_number}/comments`;

      // Prepare the request body based on Gitea's line comment API
      ...
      if (isSingleLine) {
        ...
        requestBody.start_side = side || "RIGHT";
        requestBody.line = line;
      }
      ...
      if (!commentResponse.ok) {
        ...
        throw new Error(
          `Failed to create inline comment: ${commentResponse.status} - ${errorText}`,
        );
      }

      const result = (await commentResponse.json()) as {
        id: number;
        html_url: string;
        path: string;
        line?: number;
